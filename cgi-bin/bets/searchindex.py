#!/usr/bin/python
import sys
sys.path.insert(0, "/home/parakcyy/scripts/bets")
import json
...
for i in range(0,len(singlepaths)):
    ...
    singlepaths[i]=[to_seconds(singlepaths[i][6])]+singlepaths[i]
    #print item
for i in range(0,len(doublepaths)):
    tags = ["CONNECT"]
    doublepaths[i]=[2]+doublepaths[i]
    doublepaths[i]=[tags]+doublepaths[i]
    doublepaths[i]=[to_seconds(doublepaths[i][10])]+doublepaths[i]
    #print item
masterres=singlepaths+doublepaths
masterres=sorted(masterres,key=lambda x:x[0])[:20]
